    ...
    if (data.user) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        try {
          ...
          if (memberResult.success) {
            ...
            if (
              memberResult.status === 400 &&
              memberError?.includes('등록되지 않은 인증 사용자')
            ) {
              console.log(`Retry ${i + 1}/5: 인증 사용자 UUID 등록 대기 중...`);
            } else {
              // 다른 에러는 재시도하지 않음
              console.error(
                '회원 정보 저장 실패 (non-retryable):',
                memberError
              );
              break;
            }
      ...
      if (!memberResult || !memberResult.success) {
        ...
        return {
          success: true,
          data,
          warning: `회원가입은 완료되었지만 프로필 정보 저장에 실패했습니다: ${
            memberError || '알 수 없는 오류'
          }`,
        };
      }
    }
