...
import { getMemberInfoByAuthUuid, createOrUpdateMember, checkNickname } from '../../util/memberService';
...
function MemberInfoModifyComp() {
  ...
  const [existingProfileImageFileId, setExistingProfileImageFileId] = useState(null);
  ...
  useEffect(() => {
    const loadMemberInfo = async () => {
      ...
      try {
        ...
        const name = session.user.user_metadata?.nickname || session.user.user_metadata?.full_name || '사용자';
        ...
        const email = memberData.email || session.user.email || userData.email || '';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const name =
        session.user.user_metadata?.full_name || session.user.user_metadata?.nickname || nickname || '사용자';
      ...
      if (selectedFile) {
        ...
        if (!uploadResult.success) {
          setError(uploadResult.error || '프로필 이미지 업로드에 실패했습니다.');
          setSaving(false);
          return;
        }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="flex-1 max-w-6xl mx-auto px-10 pb-24">
        ...
        {error && (
          <div className="mb-4 p-3 bg-rose-50 border border-rose-200 text-rose-600 rounded-sm text-sm">{error}</div>
        )}
          ...
          <div className="lg:col-span-3 flex justify-end mt-8">
            <button
              ...
              className={`px-8 py-2.5 text-white text-sm font-semibold rounded-sm ${
                saving ? 'bg-slate-400 cursor-not-allowed' : 'bg-amber-400 hover:bg-amber-500'
              }`}
            >
              {saving ? '저장 중...' : '수정 하기'}
            </button>
